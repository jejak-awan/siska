...
class PresensiSDService
{
    ...
    {
        $query = PresensiSD::with('siswa.user');
    ...
    public function createAttendance(array $data)
    {
        ...
        $existingPresensi = PresensiSD::where('id_siswa', $data['id_siswa'])
        ...
        return PresensiSD::create($data);
    }

    /**
     * Update attendance record
     */
    public function updateAttendance(PresensiSD $presensi, array $data)
    {
     ...
     */
    public function deleteAttendance(PresensiSD $presensi)
    {
    ...
    public function bulkCreateAttendance(array $data)
    {
        ...
        try {
            ...
            foreach ($data['presensi_data'] as $presensiData) {
                // Check if attendance already exists
                $existingPresensi = PresensiSD::where('id_siswa', $presensiData['id_siswa'])
                    ->where('tanggal', $data['tanggal'])
                    ->first();

                if (!$existingPresensi) {
                    $presensi = PresensiSD::create([
    ...
    {
        return PresensiSD::with('siswa.user')
    ...
    {
        $query = PresensiSD::where('id_siswa', $idSiswa);
